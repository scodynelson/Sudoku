...
public class SudokuCellService {
	...
	public static void draw(final SudokuCell sudokuCell, final Graphics g, final int x, final int y, final int widthHeight, final int cellPosition) {
		...
		final boolean isInitial = sudokuCell.isInitial();
		final Integer value = sudokuCell.getValue();
		final Integer guessValue = sudokuCell.getGuessValue();

		if (isInitial) {
			final String s = Integer.toString(value);

			final BufferedImage image = createImage(font, frc, widthHeight, s, sudokuCell.isInitial());
			...
			final List<String> list = concatenatePossibleValues(sudokuCell);
